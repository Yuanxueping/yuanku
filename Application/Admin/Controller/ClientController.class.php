...
class ClientController extends Controller {
    ...
    public function feedback_list(){
        ...
        $page_count = 3;    //每页的数据数
        $page_num = I('page_num') > 0 ? I('page_num') : 1; //获取ID值，没有则默认为1
        $fb_total_num = $fb_m -> count();   //获取数据总数
        $start_index = ($page_num - 1) * $page_count;//从第几条数据查起
        $total_num = ceil($fb_total_num/$page_count);//总共多少页
        ...
        for ($i = 1; $i <=$total_num ; $i++) { 
            ...
            $page_html.="<a href=".U('Client/feedback_list','page_num='.$i)." class='btn btn-default ".$active."'>".$i."</a>";
        }

        $fb_list = $fb_m ->limit($start_index,$page_count)-> select();
